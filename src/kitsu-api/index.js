...
{
  console.log(query, type); // eslint-disable-line no-console
  const subType = type.includes('MOVIE') ? 'movie' : type;
  try
  {
...
const addKitsuMetadata = async (title, eps, type) =>
{
  ...
  try
  {
    ...
    const o = [];
    for (let i = 0; i < twistData.length; i += 1)
    {
      const value = twistData[i];
      o.push({
        ...value,
        synopsis: kmeta[i].synopsis,
