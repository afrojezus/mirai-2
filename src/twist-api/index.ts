...
const load = async () => {
  ...
  try {
    ...
    const que = items.map((element: TwistListElement, index: number) => {
      return output.push({
        name: element.title,
        ongoing: false,
        link: element.link,
 ...
 */
const get = async (query: string, ongoing: boolean) => {
  const output = [] as Array<any>;
  const data = {
    uri: Proxy2 + query,
  ...
  try {
    ...
    const que = source('div.episode-list ul')
      ...
      .each(async (index: number, e: object) => {
        const ep =
          parseInt(
            source(e)
              .find('a')
...
const getSource = async (ep: number) => {
  const data = {
    uri: Proxy2 + ep,
    transform: (body: CheerioElement) => Cheerio.load(body),
  };
