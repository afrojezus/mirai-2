...
import sample from "../assets/sample.webm";
...
class Show extends Component {
  ...
  executeTwist = async reload => {
    ...
    /*if (dbval && Object(dbval.val())[this.state.id]) {
      ...
      // Sample for video functionality
      return this.setState({
        epError: false, eps: [
          {
            airdate: "2022-08-07",
            ...
            provider: "Mirai",
            season: 1,
            synopsis: "An example sample video to test the video player functionality",
            thumb: {
              original: "https://media.kitsu.io/anime/10780/poster_image/large-a5d92d5a0880401fee7059cfbafcd6d4.jpeg"
            }
          }
        ]
      });
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    />
                    <Typography className="artworktitle" variant="h4">
                      {mir && mir.play && mir.play.meta.id === data.Media.id ? (
                        lang.show.playing
                      ) : data.Media.status.includes("NOT_YET_RELEASED") ? (
                        "TBA"
                ...
                <Grid item xs className={classes.mainFrame}>
                    ...
                    {data.Media.type.includes("ANIME") ? (
                      ...
                      >
                        ...
                        {!data.Media.status.includes("RELEASING")
                          ? `• ${data.Media.startDate.year}` +
                          `• ${data.Media.chapters} ${lang.show.chapters
                    ...
                    >
                      ...
                      >
                        ...
                        {data.Media.nextAiringEpisode
                          ...
                          )} ${lang.show.episodeWhen} ${data.Media.nextAiringEpisode.episode
                          } ${lang.show.arrives}`
                          : null}
                      </Typography>
                    </Tooltip>
                  ...
                  />
                  {data.Media.rankings ? (
                    <Grid container style={{ marginTop: theme.spacing() }}>
                ...
                >
                  ...
                  {data.Media.type.includes("MANGA") ||
                    ...
                    <Button
                      ...
                      onClick={() =>
                        window.open(
                          `http://anichart.net/${`${data.Media.season.toLowerCase()}-${data.Media.startDate.year
                          }`}`
                        )
                      }
                  ...
                  {data.Media.type.includes("MANGA") ||
                    ...
                    data.Media.format.includes("OVA") ? null : !eps &&
                      !epError ? (
                    <CircularProgress
                      style={{ color: hueVib, padding: 0, width: 24, height: 24, marginLeft: 16 }}
                      className={classes.commandoTextBox}
                    />
                  ) : epError ? (
                    <FadeIn>
                      ...
                      <ItemContainer>
                        {data.Media.characters.edges.map((cast, index) => (
                          <PeopleButton
                            ...
                            role={
                              cast.voiceActors && cast.voiceActors.length > 0
                                ? `${lang.show.as} ${cast.node.name.last
                      ...
                      {data.Media.relations &&
                        data.Media.relations.edges.map((anime, index) => (
                          <CardButton
                            ...
                            subtitle={`${anime.node.type
                              } ${anime.relationType.replace(/_/gi, " ")}`}
                            onClick={() =>
                              this.openEntity(
                                `/show?${anime.node.type.includes("ANIME") ? "s" : "m"
                                }=${anime.node.id}`
                              )
                            }
                          />
                        ...
                        TwistFilter(
                          ...
                          .map((anime, index) => (
                            <CardButton
                              ...
                              onClick={() =>
                                this.openEntity(
                                  `/show?${anime.type.includes("ANIME") ? "s" : "m"
                                  }=${anime.id}`
                                )
                              }
                            />
