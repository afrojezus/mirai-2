...
class Fig extends Component
{
  ...
  vibrance = () =>
  {
    ...
    this.props.sendTitleToMir(this.state.type.includes('CHARACTER')
      ? nameSwapper(
    ...
    return Colorizer(`https://cors-anywhere.herokuapp.com/${image}`).then(pal =>
      this.setState(
        {
  ...
  unlike = async () =>
  {
    ...
    {
      this.props.firebase
        .remove(`users/${this.props.profile.userID}/favs/${entity}/${this.state.id}`)
...
export default firebaseConnect()(connect(({ firebase: { profile }, mir }) => ({ profile, mir }), mapPTS)(M.withStyles(style)(Fig)));
