...
const SuperTable = class extends React.Component
{
  ...
  goBack = (e) =>
  {
    ...
    const calculateWidth = this.props.single
      ...
      : windowWidth > 1366 ? 3 : windowWidth > 1000 ? 2 : 1;
    scrollSlide -= listelement.scrollWidth * calculateWidth;
    const newSlide = scrollSlide < 0 ? (scrollSlide = 0) : (scrollSlide);
    this.setState({ currentSlide: newSlide }, () =>
    {
      this.scroll(list);
    });
  ...
  scroll(list)
  {
    ...
    window.navigator.userAgent.indexOf('Edge') > -1
      ? (curList.scrollLeft = this.state.currentSlide)
      : curList.scroll({ left: this.state.currentSlide, behavior: 'smooth' });
  }

  render()
  {
    const {
      classes, theme, loading, data, single, ...props
    } = this.props;
    ...
    const calculateWidth = windowWidth > 1366 ? 3 : windowWidth > 1000 ? 2 : 1;
    if (loading)
    {
      return (
          ...
          >
            {data.splice(0, props.limit).map((anime, index) => (
              <GridListTile
                ...
                key={index}
                onClick={() =>
                  props.changePage(`/show?${props.type}=${
                ...
                <div className={classes.bigCardRow}>
                  ...
                  <div className={classes.bigCardText}>
                    ...
                    >
                      ...
                      {props.typeof === 'ongoing'
                        ? props.type.includes('m')
                          ? null
                          : `${timeFormatter(anime.nextAiringEpisode.timeUntilAiring)} till Episode ${anime.nextAiringEpisode.episode}`
                        : props.typeof === 'ranking'
                          ? anime.category
                          : props.typeof === 'progress'
                    ...
                    <Dotdotdot clamp={3}>
                      ...
                      >
                        {props.typeof === 'progress'
                          ? `Last watched ${moment(anime.recentlyWatched).from(Date.now())}`
                          : null}
                      </Typography>
                    </Dotdotdot>
                  </div>
                    ...
                    <Dotdotdot clamp={3}>
                      ...
                      >
                        {props.typeof === 'ongoing'
                          ? props.type.includes('m')
                            ? null
                            : `${timeFormatter(anime.nextAiringEpisode.timeUntilAiring)} till Episode ${
...
export default connect(({ routing }) => ({ routing }), mapDispatchToProps)(withStyles(style, { withTheme: true })(SuperTable));
