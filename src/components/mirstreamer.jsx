...
class MirStreamer extends React.Component
{
  ...
  componentDidMount = async () =>
  {
    if (!isEmpty(this.props.profile) && this.props.profile)
    {
      ...
      if (id.s)
      {
        return this.joinStream(id.s);
      }
      return this.makeStream(this.props.history.location.state);
    }
    return null;
  };
  ...
  makeStream = async (streamData) =>
  {
    ...
    if (db)
    {
      db
        ...
        .then(async () =>
          this.startStream(streamData.eps));
    }
    return null;
  };
  ...
  render()
  {
    ...
    return (
      ...
      >
        ...
        <div aria-role="button" onClick={this.playPause}>
          <ReactPlayer
            id="player"
            ref={(player) =>
{
...
export default firebaseConnect()(connect(({ firebase: { profile }, mir }) => ({ profile, mir }), mapPTS)(withStyles(style, { withTheme: true })(MirStreamer)));
